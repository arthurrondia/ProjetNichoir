#!/bin/bash
apt update && apt dist-upgrade -y && apt install mosquitto python3 network-manager python3-pip python3-flask python3-paho-mqtt python3-sqlalchemy git mariadb-server
...
cp ProjetNichoir/pi/mosquitto.timer /lib/systemd/system

systemctl enable /lib/systemd/system/mosquitto.timer
